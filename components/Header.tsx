...
import { Mail, Phone, MapPin, Facebook, Youtube, Linkedin, Github, Globe, Download, X, User, Moon, Sun, Star, Copy, Eye, Heart, Printer, Code, Terminal, Cpu } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  // List of professional tech-themed avatars
  const avatarImages = [
  ...
  useEffect(() => {
    ...
    // Logic to update avatar carousel only when NOT interacting
    const avatarInterval = setInterval(() => {
      // Use a functional update to check state inside interval
      setAvatarIndex((prev) => {
        // We can't easily access isHovering/isHolding directly here without refs or a different approach,
        // but we'll use a safer method below with a ref.
        return prev; 
      });
    }, 4000);

    // Re-implemented interval to respect hovering
    const intervalId = setInterval(() => {
      // Check if user is interacting with avatar
      const avatarEl = document.getElementById('cv-profile-img');
      const isUserInteracting = avatarEl?.matches(':hover') || isHolding;
    ...
    // Listen for star achievement to trigger heart spin
    const handleStarAchieved = () => {
      setIsSpinningHeart(true);
      // Fast count up animation
      let count = 0;
      const targetIncrease = 50 + Math.floor(Math.random() * 50);
      const timer = setInterval(() => {
        setLikeCount(prev => prev + 1);
        count++;
        if (count >= targetIncrease) {
          clearInterval(timer);
          setIsSpinningHeart(false);
          setToastMessage('Power Up! Your popularity is rising! 🚀');
          setShowToast(true);
        }
      }, 30);
    };
  ...
  useEffect(() => {
    if (isHolding) {
      holdIntervalRef.current = window.setInterval(spawnStar, 80);
    ...
    <>
      ...
      <header className="relative w-full h-auto md:min-h-[95vh] bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-start font-sans overflow-hidden print:min-h-0 print:bg-white transition-colors duration-500 pb-16 md:pb-0" id="section-home">
        ...
        <div className="absolute top-6 right-6 z-50 flex gap-3 print:hidden">
            <a href={personal.cvLink} download={personal.cvLink.split('/').pop()} className="group flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full shadow-lg border border-white/20 text-white hover:bg-white hover:text-vn-red transition-all duration-300">
                <Download size={18} /><span className="text-sm font-medium hidden md:block">{ui.common.downloadCV}</span>
            </a>
            <button onClick={handlePrint} className="group flex items-center gap-2 px-3 py-2 bg-white/10 backdrop-blur-md rounded-full shadow-lg border border-white/20 text-white hover:bg-white hover:text-vn-red transition-all duration-300"><Printer size={18} /></button>
            ...
            <button onClick={toggleLanguage} className="group flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full shadow-lg border border-white/20 text-white hover:bg-white hover:text-blue-600 transition-all duration-300"><Globe size={18} /><span className="text-sm font-medium uppercase">{language}</span></button>
        </div>
        ...
        <div className="relative z-10 container mx-auto px-4 flex flex-col items-center pt-20 md:pt-28">
            ...
            >
                ...
                {/* Border Spin - Fixed: Added rounded-full explicitly */}
                <div className={`absolute -inset-[2px] rounded-full bg-gradient-to-tr from-vn-red via-vn-yellow to-transparent opacity-80 ${isHolding ? 'animate-spin-super-fast' : isHovering ? 'animate-spin-fast' : 'animate-spin-slow'}`}></div>

                {/* Main Image Container - Fixed: Absolute rounded-full for spinning */}
                <div className={`relative w-full h-full rounded-full overflow-hidden border-[6px] border-slate-900 shadow-2xl bg-slate-800 transition-all duration-500 ${isHolding ? 'scale-90 animate-spin-super-fast' : isHovering ? 'animate-spin-fast' : ''}`}>
                    {avatarImages.map((imgUrl, index) => (
                      <img 
                          key={index}
                          src={imgUrl}
                          alt={`${personal.name} avatar ${index}`} 
                          className={`absolute inset-0 w-full h-full object-cover rounded-full transform transition-all duration-1000 ease-in-out ${
                            avatarIndex === index ? 'opacity-100 scale-100' : 'opacity-0 scale-110 pointer-events-none'
                          }`}
                      />
                    ))}
                    ...
                    <div className={`absolute inset-0 bg-vn-red/40 flex items-center justify-center rounded-full transition-opacity duration-300 backdrop-blur-[1px] ${isHovering ? 'opacity-100' : 'opacity-0'}`}>
                         <div className={isHolding ? 'scale-150 text-vn-yellow' : 'text-white'}>
            ...
            {/* INFO BAR */}
            <div className="w-full max-w-5xl bg-white dark:bg-slate-900 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] rounded-2xl p-6 md:p-8 animate-fade-in-up delay-200 relative group overflow-visible border-t-4 border-vn-red transition-colors duration-300" style={{ transform: !isMobile ? `translateY(${scrollY * 0.05}px)` : 'none' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                    <div onClick={() => copyToClipboard(contact.phone, ui.labels.mobile)} className="flex flex-col items-center justify-center text-center p-6 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-xl hover:shadow-2xl hover:border-vn-red/40 transition-all duration-300 hover:-translate-y-2 group/item cursor-pointer relative">
                        <div className="w-12 h-12 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl flex items-center justify-center mb-3 shadow-md border border-slate-100 dark:border-slate-600 group-hover/item:scale-110 transition-transform"><Phone size={22} className="text-vn-red" /></div>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">{ui.labels.mobile}</p>
                        <span className="text-lg text-slate-800 dark:text-slate-200 font-bold font-display tracking-wide">{contact.phone}</span>
                    </div>
                    <div onClick={() => copyToClipboard(contact.email, ui.labels.email)} className="flex flex-col items-center justify-center text-center p-6 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-xl hover:shadow-2xl hover:border-vn-red/40 transition-all duration-300 hover:-translate-y-2 group/item cursor-pointer relative">
                        <div className="w-12 h-12 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl flex items-center justify-center mb-3 shadow-md border border-slate-100 dark:border-slate-600 group-hover/item:scale-110 transition-transform"><Mail size={22} className="text-vn-red" /></div>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">{ui.labels.email}</p>
                        <span className="text-lg text-slate-800 dark:text-slate-200 font-bold font-display tracking-wide truncate max-w-full px-2">{contact.email}</span>
                    </div>
                    <div className="flex flex-col items-center justify-center text-center p-6 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-xl hover:shadow-2xl hover:border-vn-red/40 transition-all duration-300 hover:-translate-y-2 group/item">
                        <div className="w-12 h-12 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl flex items-center justify-center mb-3 shadow-md border border-slate-100 dark:border-slate-600 group-hover/item:scale-110 transition-transform"><MapPin size={22} className="text-vn-red" /></div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-scale-in">
            ...
            <div className="px-6 pb-6 relative text-center">
                <div className="w-32 h-32 rounded-full border-[6px] border-white dark:border-slate-800 shadow-xl overflow-hidden -mt-16 mx-auto bg-slate-200">
                    <img src={avatarImages[avatarIndex]} alt={personal.name} className="w-full h-full object-cover rounded-full" />
                </div>
                ...
                <div className="mt-6 space-y-4 text-left">
                     <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                        <div className="flex items-center gap-3 mb-2 border-b dark:border-slate-700 pb-2"><User size={18} className="text-slate-400"/><span className="text-xs font-bold uppercase">{ui.common.basicInfo}</span></div>
